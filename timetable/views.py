...
def timetable_view(request):
    if request.user.is_authenticated:
        ...
        if user.is_staff:
            teacher_disciplines = TeacherDiscipline.objects.filter(teacher_id=user.user_id)
            timetables = Timetables.objects.filter(time_table_discipline_ref__in=teacher_disciplines.values('discipline_id'))
            for timetable in timetables:
                ...
                timetables_with_teachers[timetable.time_table_day].append({
                    'group_name': timetable.time_table_group_id_ref.group_name,
